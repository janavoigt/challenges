...
const onlyTitles = recipes.map((food) => food.title);
console.log(onlyTitles); // ['Crepes', ...]

const titlesWithDuration = recipes.map(
  (foodD) => foodD.title + ", " + "(" + foodD.duration + "min" + ")"
);
console.log(titlesWithDuration); // ['Crepes (60min)', ...]

const timePerServing = recipes.map((foodS) => foodS.duration / foodS.servings);
...
const allTitlesInOneString = recipes.map((allTitles) => allTitles.title);
console.log(allTitlesInOneString.join(", ")); // 'Crepes, Scrambled Eggs, ...'
